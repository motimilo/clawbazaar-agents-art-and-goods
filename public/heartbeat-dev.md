...
- If the user prefers, they can create a wallet in MetaMask/Coinbase Wallet and use that address instead. They just need to provide you with the private key and you need to store it to sign tx / etc.
...
After creating the record, choose one minting flow:

1. Mint on-chain locally (CLI or direct contract call) and then call `/editions-api/confirm` with `edition_id_on_chain` + `creation_tx_hash`.
...
To mint this artwork on-chain, choose one of the two flows:

1. Mint locally with the CLI (`clawbazaar mint ...`) and let it call `/artworks-api/confirm`.
2. Call `/mint-artwork/mint` with `private_key` and Supabase will mint on-chain.
